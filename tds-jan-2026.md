...
- Project / GA questions:
  ...
  - [x] Generate and add four experimental image questions `src/q-generate-affective-chart.js`, `src/q-generate-concept-incarnation.js`, `src/q-generate-style-transplant.js`, and `src/q-generate-paradox-portrait.js`. Each asks students to create and submit one AI-generated image plus one JSON metadata file, following the original brief closely, with the question text embedded directly in the JS file as markdown. Each carries weight 0.25, is marked prominently as Experimental, includes an Ask AI section, and notes that up to 0.75 bonus marks may be awarded later via offline AI evaluation of the image, including a diversity component based on image-embedding uniqueness. Verification checks that the first submitted URL is a public CORS-accessible image in an accepted format (.png, .jpg, .webp, .avif) with minimum short side 1024px, and that the second submitted URL is a public CORS-accessible JSON document matching the schema required by that specific brief.
  - [x] Generate and add the three experimental network-game questions `src/q-network-game-labyrinth.js`, `src/q-network-game-detective.js`, and `src/q-network-game-signal.js`, based on the corresponding pages at https://tds-network-games.sanand.workers.dev/. Each is weight 1, marked prominently as Experimental, includes an Ask AI section, explicitly says that students are expected to figure out the game mechanics on their own, and notes that AI coding agents mighthelp them win. Students submit the completion JWT token returned by the game as proof. Validation happens on both client and server side via `src/a-network-game-*.js`, verifying the ES256 signature against https://tds-network-games.sanand.workers.dev/.well-known/jwks.json, checking that sub matches the student email, game matches labyrinth, detection, or signal as appropriate, week_id matches the current ISO week, and completed_at is within the last 7 days.
  - [x] Capture the flag exercises: PicoCTF, TryHackMe, Kaggle. <!-- https://claude.ai/chat/320b30ef-2176-4754-82a5-c0e95394884f | https://gemini.google.com/app/84dd672bfbb75d46 -->
  - [x] Scrape a dark web .onion site
  - [ ] Reverse engineer in JavaScript: python-dateutil rrule, jmespath, whatwg URL parser, IBAN validator. https://claude.ai/chat/29c4d11e-b57e-4dc4-a680-2a76bfe93952
  - [ ] Submit a git repo that solves a problem in a single coding agent session and export the session and share it. Bonus marks for how you are prompting it
  - [ ] Respond to email provided problems in a thread with context. Each answer gives a small secret token. Collect all of them. How to prevent a human from solving then? Response time?
  ...
  - [ ] Submit/correct a LEAN proof for a maths problem (though what are they learning? maybe they necessarily need to RUN lean to validate?)
  - [ ] Compile data science package to Rust / WASM. <!-- https://gemini.google.com/app/18c9512432449699 -->
  - [ ] Bluesky firehose is structured & free. sentiment analysis, graph algorithms, anomaly detection, topic modeling, ...
  - [ ] Rank high on the agent web (test via Vane) for specific keywords. Make sure you're the 3rd, not first. <!-- https://claude.ai/chat/ce315d63-483e-4589-95e4-0df4b9b4ee7c | https://chatgpt.com/c/69c339b2-c4b4-8399-b257-56f6887c67cf -->
